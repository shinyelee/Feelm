...
public class UserDAO {
	...
	public int userCheck(String userID, String userPassword) throws Exception {
		Connection conn = null; // 자바와 DB 연결
		PreparedStatement pstmt = null; // 특정한 SQL문 수행하도록 하는 클래스 
		ResultSet rs = null;
		String SQL = "";
		...
		try {
			...
			if (rs.next()) { // DB에 있으면 다음으로 넘어감
				if (userPassword.equals(rs.getString("userPassword"))) { // DB의 userPassword와 myInfoDelete의 userPassword 일치
					x = 1; // 인증 성공
				}else
					x = 0; // 비밀번호 다름	
			} else
